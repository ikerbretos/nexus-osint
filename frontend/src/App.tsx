...
export default function App() {
    ...
    const init = async () => {
      try {
        ...
        if (res.data.length > 0) {
          ...
          const newCase = await axios.post('http://localhost:3001/api/cases', { name: 'CASO ALPHA', description: 'Investigación General' });
          setCurrentCaseId(newCase.data.id);
        }
      } catch (err) {
      ...
      sortedTypes.forEach(type => {
        ...
        groupNodes.forEach(n => {
          ...
          Object.entries(n.data).forEach(([key, val]) => {
            if (['x', 'y', 'id', 'label', 'name', 'ip', 'email', 'number', 'address'].includes(key) && val === mainLabel) return; // Skip main label redundancy
            if (['x', 'y', 'id'].includes(key)) return; // Skip coords
            if (!val || val === '') return;
  ...
  const toggleGroup = (groupKey: string) => {
    if (draggingNode) return; // Prevent toggle on drag
  ...
  const applyLayout = (type: string) => {
    setActiveLayout(type); // FIX: Ensure state updates so grouping logic reacts
    ...
    if (type === 'grid') {
      // "Categorical Waterfall" (User Request: Grid Mode = Hierarchical by Type)
      // Reference: Target Top -> Row 1 (IPs) -> Row 2 (Phones) -> Row 3 (Emails) etc.

      if (rootNode) {
        rootNode.x = cx;
        rootNode.y = cy - 300; // Start higher up
      }
      ...
      // Define standard order for OSINT layers if possible, or just keys
      const typeOrder = ['ip', 'domain', 'server', 'company', 'phone', 'email', 'identity', 'crypto', 'bank'];
      const sortedTypes = Object.keys(grouped).sort((a, b) => {
        const idxA = typeOrder.indexOf(a);
        const idxB = typeOrder.indexOf(b);
        // If both known, sort by order. If unknown, push to end.
      ...
      let currentY = (rootNode ? rootNode.y : cy - 300) + 100; // Tighter initial gap
      ...
      sortedTypes.forEach(t => {
        ...
        // Row Configuration
        // Tighter spacing to mimic Maltego density
        const spread = 120;
      ...
      // Maltego-style Radial: Concentric circles from Root
      // 1. Identify layers via BFS
      ...
      // Handle disconnected nodes (put them in outer layer)
      newNodes.forEach(n => {
      ...
      Object.entries(layerGroups).forEach(([layerStr, nodesInLayer]) => {
        ...
        nodesInLayer.forEach((n, i) => {
          if (layer === 0) {
            ...
            // Distribute circle angle range based on parent's angle to minimalize crossing? 
            // Simple full circle for now is much better than previous linear index
      ...
      // 2. Identify Neighbors (Level 1 Candidates)
      const neighbors = newNodes.filter(n => n.id !== rootId);

      // 3. Group by Type (Slots)
      const grouped: { [key: string]: any[] } = {};
      neighbors.forEach(n => {
        // Determine Sort Key (Type)
        if (!grouped[n.type]) grouped[n.type] = [];
        grouped[n.type].push(n);
      });

      // 4. Create Slots (One per Type)
      // FIX: Sort types based on current visual position (Average X) to preserve order
      // instead of alphabetical sort which causes jumps.
      ...
      const levelY = (rootNode ? rootNode.y : cy - 250) + 180; // Level 1

      // 5. Position Nodes into Slots
      types.forEach((t, i) => {
        const items = grouped[t];
        const slotX = startX + (i * spacing);

        // STACK all items in the slot. 
        // If >1, they form a Group Node at this exact center.
        // If =1, the single node sits here.
        items.forEach(n => {
          n.x = slotX;
          n.y = levelY;
        });
  ...
  const layoutData = useMemo(() => {
    ...
    nodes.forEach(target => {
      ...
      Object.entries(groups).forEach(([type, items]: [any, any], typeIdx) => {
        ...
        // In 'grid' mode, we want ALL nodes visible in their rows, NO grouping.
        // In other modes (tree, radial), we group if > 1.
        const shouldGroup = items.length > 1 && activeLayout !== 'grid';
        ...
        if (shouldGroup) {
          ...
          if (items.length > 0) {
            // 1. STRICT CENTROID LOGIC (User Request: "Stay in original place")
            ...
            if (activeLayout === 'tree') {
              // FORCE GROUP TO STAY IN MAIN ROW (Level 1)
              // Do not sink with children
              vx = cx;
              vy = target.y + 180; // Fixed "Row 1" height from applyLayout
            } else {
              vx = cx;
              vy = cy;
            }

            // Edge case: If they are all at 0,0 (just added), fall back to Layout Default
            if (activeLayout !== 'tree' && Math.abs(vx) < 1 && Math.abs(vy) < 1) { // Skip 0 check for tree if forced
  ...
  // AUTO-SPREAD CHILDREN ON EXPAND (MIND MAP STYLE + COLLISION AVOIDANCE)
  useEffect(() => {
    ...
    // 1. Identify all "Obstacles" (Nodes that are NOT part of the active expansion groups)
    // We want to avoid these.
    const activeChildIds = new Set<string>();
    expandedKeys.forEach((k: unknown) => { // Fix TS Key error
    ...
    expandedKeys.forEach((k: unknown) => {
      ...
      if (children.length > 0) {
        ...
        if (manual) {
          ...
          // STRICT ANCHOR LOGIC
          // Use the first child as the anchor.
          // This ensures visually stable expansion from the group icon.
          if (children.length > 0) {
        ...
        // 2. Decide Pattern based on Layout
        if (activeLayout === 'tree') {
          // VERTICAL LIST STYLE (User Request: "hacia abajo se vean")
          // Children line up vertically below the group
          const startY = oy + 100; // Drop distance
          const verticalGap = 80;

          children.forEach((c, i) => {
            let tx = ox; // Same X as group (Straight down)
            let ty = startY + (i * verticalGap);

            // Avoid overlapping with existing nodes loosely
            // But prioritize strict vertical alignment
  ...
  return (
    <div className="flex h-screen bg-[var(--color-canvas-bg)] text-neutral-300 font-sans selection:bg-cyan-500/20">

      {/* SIDEBAR */}
      <aside className="w-16 bg-[var(--color-panel-bg)] border-r border-[var(--color-border)] flex flex-col items-center py-4 z-40">
        <div className="flex-1 w-full px-2 space-y-4 overflow-y-auto hide-scrollbar mt-4">
          <div className="space-y-2">
            <div className="text-[9px] font-bold text-neutral-600 uppercase text-center tracking-widest mb-1">Elementos</div>
            {Object.entries(ENTITY_CONFIG).map(([type, config]: [any, any]) => (
              <button key={type} onClick={() => { setMode('add'); setAddType(type); }}
                className={`w-full aspect-square rounded-lg flex items-center justify-center transition-all duration-200 group relative
                    ${mode === 'add' && addType === type ? 'bg-white/10 text-white shadow-[0_0_10px_rgba(255,255,255,0.1)]' : 'text-neutral-500 hover:bg-white/5 hover:text-neutral-300'}`}
                title={config.label}>
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden">
        {/* HEADER */}
        <header className="h-14 bg-[var(--color-panel-bg)] border-b border-[var(--color-border)] flex items-center justify-between px-6 z-30 shrink-0 relative">
          <div className="flex items-center gap-4">
            <div className="w-8 h-8 flex items-center justify-center bg-cyan-950/30 rounded border border-cyan-500/20 text-cyan-400">
              <Shield size={16} />
            </div>
            <h1 className="font-bold text-lg tracking-widest text-white uppercase italic">NEXUS <span className="text-neutral-600 font-mono text-[10px] not-italic tracking-normal">v9.5</span></h1>
          </div>
          ...
          <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-96">
            <div className="relative group">
              ...
              <input type="text" placeholder="Búsqueda de inteligencia..."
                className="w-full bg-[#111] border border-[var(--color-border)] rounded-full py-1.5 pl-10 pr-4 text-xs text-neutral-300 focus:outline-none focus:border-cyan-800 focus:bg-black transition-all" />
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 relative">
            ...
            {showExportMenu && (
              <div className="absolute top-full right-0 mt-2 w-48 glass-panel rounded-lg shadow-xl overflow-hidden z-50 flex flex-col">
        ...
        {/* TOOLBAR */}
        <div className="absolute top-20 left-6 z-30 glass-panel rounded-lg flex items-center p-1 gap-1">
          <button onClick={() => setMode('select')} className={`tool-btn ${mode === 'select' ? 'active' : ''}`} title="Seleccionar"><MousePointer2 size={16} /></button>
          <button onClick={() => setMode('link')} className={`tool-btn ${mode === 'link' ? 'active' : ''}`} title="Conectar"><LinkIcon size={16} /></button>
          <button onClick={() => setMode('eraser')} className={`tool-btn ${mode === 'eraser' ? 'active' : ''}`} title="Eliminar"><Eraser size={16} /></button>
        </div>

        {/* LAYOUT CONTROLS (BOTTOM-LEFT) */}
        <div className="absolute bottom-6 left-6 z-30 glass-panel rounded-lg flex items-center p-1 gap-1">
        ...
        {/* WORKSPACE */}
        <div ref={workspaceRef} className="flex-1 relative cursor-crosshair overflow-hidden bg-[var(--color-canvas-bg)]"
          onMouseDown={(e) => {
            if (mode === 'add') {
          ...
          onMouseMove={(e) => {
            ...
            // Current mouse pos in canvas coords
        ...
        >
          ...
          <div className="absolute inset-0 pointer-events-none opacity-[0.07]"
            style={{ backgroundImage: 'radial-gradient(#fff 1px, transparent 1px)', backgroundSize: `${30 * scale}px ${30 * scale}px`, backgroundPosition: `${offset.x}px ${offset.y}px` }} />
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none z-0">
            <g transform={`translate(${offset.x}, ${offset.y}) scale(${scale})`}>
              {layoutData.displayLinks.map((l, i) => {
                ...
                // Bezier Curve Logic
                // If it's a "Tree" or "MindMap" feel, we want curves starting horizontally or vertically depending on orientation.
                // General "Organic" Curve: Control points at 1/3 and 2/3
                // But simplified: A Quadratic or Cubic curve pulling towards the center?

                // Let's use a standard horizontal-ish bezier if dx > dy (Side to side)
                // Or vertical bezier if dy > dx (Top to bottom)
                // Simplest robust organic curve:

                const dist = Math.sqrt(Math.pow(t.x - s.x, 2) + Math.pow(t.y - s.y, 2));
                // const cpOffset = dist * 0.3;

                // We don't have directionality easily here (parent/child).
                // Assuming Source is closer to root... not always true in React Flow style data.
                // Let's just draw simple straight lines for now to ensure cleanliness as requested "don't cross".
                // User said "fijate en el diseño... curvas". The image has curves.
                // Let's try to infer direction from 0,0 (Center)

                // Vector from Center (approx) to Source
                // This is getting complex for a stateless render.
                // Let's stick to straight lines but slightly smoother or just straight.
                // Image showed curved lines.
                // Let's try to implement a simple curved path.

                return <path key={i}
                  d={`M ${s.x} ${s.y} L ${t.x} ${t.y}`} // Stick to Line for now, safer to avoid "crossing" confusion until we have robust routing.
                  stroke="#333" strokeWidth="1" fill="none"
                />;

                // Actually, let's keep it straight L. The user complained about crossing. Straight minimizes visual crossing complications compared to naive curves.
                // If I implement bad curves, it will look worse.
                // I will add 'opacity' to make them subtle.
              })}
            </g>
          </svg>
                ...
                <div key={node.id}
                  onMouseDown={(e) => {
                    ...
                    if (mode === 'link') {
                      ...
                      setSelectedNodeId(node.id); setDraggingNode(node.id); setIsEditing(true); setShowAdvancedProps(false);
                    }
                  }}
                  onClick={(e) => {
                    e.stopPropagation();
                  ...
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center bg-[#0a0a0a] border transition-all duration-300 relative
                       ${isSel ? 'border-white shadow-[0_0_20px_rgba(255,255,255,0.15)] ring-1 ring-white/20' : 'border-neutral-800 group-hover:border-neutral-600'}
                       ${linkingSource === node.id ? 'ring-2 ring-emerald-500' : ''}
                       ${isCollapsed ? 'ring-2 ring-red-500/50' : ''}`}>
                    {React.createElement(display.Icon, { size: 18, style: { color: display.color } })}
                  ...
                  <div className={`mt-2 px-2 py-0.5 rounded bg-black/80 border border-white/10 backdrop-blur-sm text-[9px] font-bold tracking-wider text-neutral-300 pointer-events-none transition-opacity
        ...
        {selectedNodeId && isEditing && getNodeInfo(selectedNodeId) && (
          <div className="absolute top-20 right-6 w-96 glass-panel rounded-xl flex flex-col z-50 animate-slide-down shadow-2xl max-h-[calc(100vh-6rem)] flex flex-col">
            <div className="p-4 border-b border-white/10 flex justify-between items-center bg-[#0a0a0a]/95 backdrop-blur-md shrink-0 rounded-t-xl">
              <span className="text-[10px] font-bold uppercase tracking-widest text-neutral-400">Propiedades del Elemento</span>
              ...
              <div className="space-y-3">
                {ENTITY_CONFIG[getNodeInfo(selectedNodeId)!.type].fields.slice(0, 5).map((f: any) => (
                  <div key={f.key} className="space-y-1">
                    <label className="text-[9px] text-neutral-500 font-bold uppercase block tracking-wider">{f.label}</label>
                    <input type="text" value={getNodeInfo(selectedNodeId)!.data[f.key] || ''}
                      ...
                      className="w-full bg-[#111] border border-white/10 rounded px-2 py-1.5 text-xs text-white focus:border-cyan-800 focus:bg-black outline-none transition-all placeholder-neutral-700"
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-3 animate-slide-down">
                      {ENTITY_CONFIG[getNodeInfo(selectedNodeId)!.type].fields.slice(5).map((f: any) => (
                        <div key={f.key} className="space-y-1">
                          ...
                          <input type="text" value={getNodeInfo(selectedNodeId)!.data[f.key] || ''}
                            onChange={(e) => setNodes(p => p.map(n => n.id === selectedNodeId ? { ...n, data: { ...n.data, [f.key]: e.target.value } } : n))}
                            className="w-full bg-[#111] border border-white/10 rounded px-2 py-1 text-[11px] text-neutral-300 focus:border-cyan-800 outline-none transition-colors" />
                        </div>
                      ))}
                    </div>
                  )}
            ...
            <div className="p-4 border-t border-white/10 bg-[#0a0a0a]/95 shrink-0 rounded-b-xl flex gap-2">
              <button onClick={() => handleEnrich(selectedNodeId)} className="flex-1 py-1.5 bg-cyan-950/40 border border-cyan-900/50 text-cyan-400 text-[10px] font-bold uppercase tracking-wider rounded hover:bg-cyan-900/60 hover:text-cyan-300 transition shadow-lg shadow-cyan-900/10 flex items-center justify-center gap-2">
                <Shield size={12} /> Enriquecer
              </button>
        ...
        {/* ZOOM CONTROLS */}
        <div className="absolute bottom-6 right-6 flex flex-col gap-1 glass-panel rounded-lg p-1">
        ...
        {showSettings && (
          <div className="absolute inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center">
            {/* ... (Settings Content) ... */}
            {/* (I'll keep this part, just wrapping the new render logic) */}
            <div className="w-[500px] glass-panel rounded-xl shadow-2xl p-6 border border-white/10 relative">
              {/* ... Existing Settings ... */}
